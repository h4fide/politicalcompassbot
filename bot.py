...
def my_resultes(message):  
    ...
    if db.check_reslt(message.from_user.id) == False:
        ...
        wait = bot.send_message(message.chat.id, eval(f'lang.wait_msg_{choosenlang}'), reply_markup=types.ReplyKeyboardRemove())
        wait
...
def question2(message):
    ...
    if message.text == '/cancel':
        ...
        if catn(message.text, message.from_user.id) == 'error':
            bot.send_message(message.chat.id, f'{func_wronginput(message.from_user.id)}', reply_markup=rkm(message.from_user.id))
            bot.register_next_step_handler(message, question2)
        else:
